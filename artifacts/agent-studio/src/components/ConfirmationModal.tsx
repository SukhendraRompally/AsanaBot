...
export function ConfirmationModal() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { e.preventDefault(); confirmAction(false); }
      else if (e.key === 'Enter') { e.preventDefault(); confirmAction(true); }
    };
      ...
      {req && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-card border border-destructive/20 shadow-[0_0_40px_rgba(220,38,38,0.12)] rounded-2xl overflow-hidden"
          >
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="p-4 bg-muted/50 border border-border rounded-lg">
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 block">What will happen</span>
                <p className="text-sm text-foreground/90 font-medium leading-relaxed">{req.message}</p>
              </div>

              <div className="text-[11px] text-muted-foreground text-center">
                Press <kbd className="px-1.5 py-0.5 rounded bg-muted border border-border font-mono">Enter</kbd> to confirm
                or <kbd className="px-1.5 py-0.5 rounded bg-muted border border-border font-mono">Esc</kbd> to cancel
              </div>
